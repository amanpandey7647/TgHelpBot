...
async def reading_tof(BOTIQUE, entity, message, is_out=None):
    """
    ...
    dialog = (await BOTIQUE(GetPeerDialogsRequest([entity]))).dialogs[0]
    max_id = dialog.read_outbox_max_id if is_out else dialog.read_inbox_max_id
    return message_id <= max_id


async def get_target_self_message(BOTIQUE, event):
    """
    ...
    if event.is_reply and target.out:
        return target
    if not target:
        return await get_recent_self_message(BOTIQUE, event)


async def get_recent_self_message(BOTIQUE, event):
    async for message in BOTIQUE.iter_messages(
